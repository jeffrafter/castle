class Subscription < ActiveRecord::Base
  ...
  named_scope :needy, lambda {|channel_id| {
    :select => "subscriptions.user_id AS user_id, subscriptions.channel_id AS channel_id, subscriptions.number_per_day AS number_per_day, IFNULL(deliveries.delivery_count, 0) AS delivery_count",
